...
public class nQueens {
  ...
  //now simply call the function nQueens
    ...
    public static void solver(int n, int cloc, int queensPlaced, boolean[][] board, List<List<String>> ans, int[] rows, int[] cols, int[] d1, int[] d2){
        ...
        for(int i= cloc; i < n * n; i++){
            int row = i/n;
            int col = i%n;
            if(!(rows[rows] == 1 || cols[col] == 1 || d1[row + col] == 1 || d2[row - col + n - 1] == 1)){
                board[row][col] = true;
                rows[row] = 1;
                cols[col] = 1;
                d1[row + col] = 1;
                ...
                solver(n, i + 1, queensPlaced + 1, board, ans, rows, cols, d1, d2);                
