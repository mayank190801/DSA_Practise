...
public class subsequencesSimilar{
    ...
    public static void solveSimilar(String str, int index, String temp, char last, int taken){
        if(index == str.length()){
            System.out.println(temp);
            return;
        } 

        //perfect logic no need to check baby
        if(taken == 1){
            solveSimilar(str, index+1, temp + str.charAt(index), str.charAt(index), 1);  
            solveSimilar(str, index+1, temp, str.charAt(index), 0);
