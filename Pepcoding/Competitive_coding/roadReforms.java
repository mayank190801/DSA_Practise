...
public class roadReforms{
    ...
    public static void main(String[] args) {
        ...
        //code here
        //no need of making tree according to me
        int test = sc.nextInt();
        while(test --> 0){
        	int n = sc.nextInt();
        	int m = sc.nextInt();
        	...
        	//after this simply think what you can do you know - think think think
        	Arrays.sort(data, (a,b) -> Long.compare(a[2], b[2]));
        	
			//after the data is sorted, let's figure something else out
			// System.out.println(Arrays.deepToString(data));
			DisjointUnionSets dsu = new DisjointUnionSets(n + 1);  //and bikhre hue honge n

			long maxVal = 0;
			long sum = 0;
			long smaller = Integer.MIN_VALUE;
			...
			for(int i = 0; i < m; i++){
				...
				long speed = data[i][2];
				if(speed <= k){
					smaller = max(smaller, speed);
				}

				if(speed > k){
					bigger = min(bigger, speed);
				}

				//let's find their parent
				int par1 = dsu.find(x);
				int par2 = dsu.find(y);
				...
				//otherwise simply connnect them					
			...
			//after this we will check if maxVal is under k or ont
			if(maxVal > k){
				System.out.println(sum);	
			}else{
				System.out.println(min(k - smaller, bigger - k));
			}
        }
