...
public class may15que2{
    ...
    public static void main(String[] args) {
        ...
        while(test --> 0){
            ...
            long[] arr = new long[n];
            for(int i= 0; i < n; i++){
                arr[i] = sc.nextLong();   //pretty simple and obvious
            }

            //now moving forward  -- never say never
            int st = 0;
            int en = n-1;            

            while(st <= en){
                int mid = (st + en)/2;
                if(!checkExistence(arr, st, mid)){
                    en = mid-1;
                }else{
                    st = mid+1;
                }        
    ...
    public static boolean checkExistence(long[] arr, int st, int en){
        Scanner sc = new Scanner(System.in);
