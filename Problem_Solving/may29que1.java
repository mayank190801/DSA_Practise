...
public class may29que1{
    ...
    public static void main(String[] args) {
        ...
        //code here -- figure this question out 
        //should be easy enough, it is more about are you able to code this stuff?? -- older me could code this in actually 4-5 mins
        int t = sc.nextInt();
        while(t --> 0){
            int n = sc.nextInt();
            int m = sc.nextInt();

            ArrayList<Integer>[] graph = new ArrayList[n + 1];  //make the graph, simple  -- yehi theek hai shayad approach
            for(int i = 0; i <= n; i++){
                graph[i] = new ArrayList<>();  //initate proces begins 
            }
            ...
            for(int i = 1; i <= n; i++){
                map.putIfAbsent(graph[i].size(), 0);
                map.put(graph[i].size(), map.get(graph[i].size()) + 1);                                
            }
            
            int ans1 = 0;
            int ans2 = 0;
            for(int key : map.keySet()){
                //it is saying the value never got change to be set!!
                //always 3 values are expected simple -- think about it. 
    ...
    //this should work
    //the question arises, wouldn't there be fixed number of edges?
    //it that would have been the case, huh?
