...
public class luo {
    ...
    public static void main(String[] args) {
        ...
        while (test-- > 0) {
            ...
            ArrayList<Integer> fixed = new ArrayList<>(set);
            int ans = 0;
            int f = fixed.size();

            // edge case
            ans = Math.max(Math.min(left + f, m), Math.min(right + f, m));
            for (int i = 0; i < f; i++) {
                int num = fixed.get(i);
                int count = 1 + (Math.min(num - 1, left + i)) + (Math.min(m - num, right + (f - i - 1)));
