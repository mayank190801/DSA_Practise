    ...
    public static void main(String[] args) {
        ...
        while (test-- > 0) {
            ...
            for (int i = 0; i < n; i++) {
                ...
                list.add(new Pair(times, i));
            }
            // so now we have to sort it and simply create two arrays for our answer
            List<Integer> first = new ArrayList<>();
            List<Integer> second = new ArrayList<>();

            Collections.sort(list, (a, b) -> b.a - a.a);
            // now I have this I think we can move on a bit from here

            for (int i = 0; i < n; i++) {
                int t1 = (first.size() + 1) * s1;
                int t2 = (second.size() + 1) * s2;

                if (t1 > t2) {
                    second.add(list.get(i).b + 1);
                } else {
                    first.add(list.get(i).b + 1);
                }

            }

            // now we hvae to print those
