...
public class easeAssembly {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < blocks.size() - 1; i++) {
            map.put(blocks.get(i), blocks.get(i + 1));
        }
        map.put(blocks.get(blocks.size() - 1), Integer.MAX_VALUE);

        int split = 0;
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < towers.get(i).size() - 1; j++) {
                int get = towers.get(i).get(j);
                int next = towers.get(i).get(j + 1);
                int shouldBe = map.get(get);
        ...
        // jitne split hove utne
        int combination = n + split - 1;
        out.println(split + " " + combination);
