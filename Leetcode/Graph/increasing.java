...
public class increasing {
    ...
    public static void main(String[] args) {
        ...
        // so now we have the input, now let's move on and figure stuff outo
        int[] left = new int[n];
        int[] right = new int[n];
        ...
        for (int i = 1; i < n; i++) {
            int u = arr[i - 1];
            int v = (i + 1) == n ? Integer.MAX_VALUE : arr[i + 1];
            int curr = arr[i];

            if (u < curr || (u >= curr && curr < v)) {
                left[i] = left[i - 1] + 1;
            } else {
                left[i] = left[i - 1];
            }
        ...
        for (int i = n - 2; i >= 0; i--) {
            int u = arr[i + 1];
            int v = (i - 1) == -1 ? Integer.MIN_VALUE : arr[i - 1];
            int curr = arr[i];
        ...
        // now let's solve it from here brother
        for (int i = 0; i < q; i++) {
            int l = queries[i][0];
            int r = queries[i][1];
            if (l == r) {
            ...
            int total = left[n - 1];
            total = total - (left[l - 1] - 1) - (right[r - 1] - 1);
            out.println(total);
        }
        out.close();
    }
