...
public class morningStar {
    ...
    public static void main(String[] args) {
        ...
        while (test-- > 0) {
            ...
            // now we have to make sure of something as well brother, umm let's think what
            // could be done
            Map<Integer, Integer> start = new HashMap<>();
            Map<Integer, Integer> end = new HashMap<>();
            Map<Integer, Integer> subtract = new HashMap<>();
            ...
            for (int i = 0; i < n; i++) {
                ...
                // now let's think how we can figure out some stuff at this position, let's
                // think about it
                // let's see how could we solve this problem brother - without using IDE - you
                // should be able to do
                int sum = u + v;
                int sub = u - v;

                start.putIfAbsent(u, 0);
                end.putIfAbsent(v, 0);
                subtract.putIfAbsent(sub, 0);
                add.putIfAbsent(sum, 0);

                // now we have the initial values
                start.put(u, start.get(u) + 1);
                end.put(v, end.get(v) + 1);
                subtract.put(sub, subtract.get(sub) + 1);
                add.put(sum, add.get(sum) + 1);

                // we have the values we wanted, even thought there is some learning curve maybe
                // one day I will get good at this
            }

            // now for each point I just have to find the connect and add them
            for (int i = 0; i < n; i++) {
                int u = arr[i][0];
                int v = arr[i][1];

                count += start.get(u) + end.get(v) + subtract.get(u - v) + add.get(u + v) - 4;
            }
