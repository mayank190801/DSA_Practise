...
public class nastya {
    ...
    public static void main(String[] args) {
        ...
        while (test-- > 0) {
            ...
            long[] finalCost = new long[n + 1];
            int[] vis = new int[n + 1];
            while (!que.isEmpty()) {
                int curr = que.poll();
                // refractor this below code to look its beset
                if (set.contains(curr)) {
                    finalCost[curr] = 0;
                } else {
                    if (vis[curr] == 1) {
                        finalCost[curr] = Math.min(finalCost[curr], value[curr - 1]);
                    } else {
                        finalCost[curr] = value[curr - 1];
                    }
                }
                for (int edges : graph[curr]) {
                    vis[edges] = 1;
                    finalCost[edges] += finalCost[curr];
                    indegree[edges]--;
                    if (indegree[edges] == 0)
                        que.add(edges);
                }
