...
public class password {
    ...
    public static void main(String[] args) {
        ...
        while (test-- > 0) {
            ...
            int m = sc.nextInt();
            char[] numOne = sc.nextLine().toCharArray();
            char[] numTwo = sc.nextLine().toCharArray();
            ArrayList<Set<Integer>> levels = new ArrayList<>();
            for (int i = 0; i < m; i++) {
                levels.add(new HashSet<>());
            }
            boolean flag = true;
            for (int i = 0; i < m; i++) {
                int start = numOne[i] - '0';
                int end = numTwo[i] - '0';
                if (start > end) {
                    flag = false;
                    break;
                }
                Set<Integer> set = levels.get(i);
            ...
            int curr = 0;
            Set<Integer> set = levels.get(curr);
            for (char ch : inp) {
                int num = ch - '0';
                if (set.contains(num)) {
                    set.remove(num);
