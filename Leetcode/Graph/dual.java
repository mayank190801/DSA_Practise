...
public class dual {
    ...
    public static void main(String[] args) {
        ...
        while (test-- > 0) {
            ...
            // so now we have the array, let's think - basically find the biggest number

            int bigPIdx = -1, bigNIdx = -1;
            for (int i = 0; i < n; i++) {
                if (arr[i] > 0)
                    bigPIdx = i;
                if (arr[i] < 0)
            ...
            if (bigPIdx == bigNIdx && bigPIdx == -1) {
                out.println(0);
                continue;
            }

            // apply these similar operations on both of the cases
            ArrayList<Pair> ans = new ArrayList<>();
            int selectIdx = bigPIdx != -1 ? bigPIdx : bigNIdx;
            for (int i = 0; i < 9; i++) {
                ans.add(new Pair(selectIdx + 1, selectIdx + 1));
            }
            for (int i = 0; i < n; i++) {
                ans.add(new Pair(i + 1, selectIdx + 1));
            }

            // now only one easy pease logic if all positive and all negative
            if (bigPIdx != -1) {
                // we know all positive,
                for (int i = 1; i < n; i++) {
