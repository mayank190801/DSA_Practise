...
public class selfie {
    ...
    public static void main(String[] args) {
        ...
        while (test-- > 0) {
            ...
            int[] lines = new int[n];
            int[][] parabola = new int[m][3];
            for (int i = 0; i < n; i++) {
                lines[i] = sc.nextInt();
            }
            for (int i = 0; i < m; i++) {
                for (int j = 0; j < 3; j++) {
                    parabola[i][j] = sc.nextInt();
                }
            }
            sort(lines);
            for (int i = 0; i < m; i++) {
                ...
                long deter = 4L * (long) a * (long) c;
                if (deter < 0) {
                    out.println("NO");
                    continue;
                }

                double rootedDeter = Math.sqrt(deter);
                long left = (long) Math.floor((double) b - rootedDeter);
                long right = (long) Math.ceil((double) b + rootedDeter);

                // find the idx greater than left
                int idx = -1;
                int l = 0, r = n - 1;
                while (l <= r) {
                    int mid = (l + r) / 2;
