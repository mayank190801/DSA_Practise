...
public class walkaway {
    ...
    public static void main(String[] args) {
        ...
        while (test-- > 0) {
            ...
            int total = 0;
            int[] contri = new int[m];

            // yaha ek calculation perform krni reh gyi
            int startContri = seller.get(0) == 1 ? 0 : 1 + ((seller.get(0) - 2) / d);
            total += startContri;

            for (int i = 0; i < m; i++) {
                int st = seller.get(i);
                int en = i != (m - 1) ? seller.get(i + 1) : n + 1;
                int spaces = en - st;
                int cookie = 1 + ((spaces - 1) / d);
                contri[i] = cookie;
                total += cookie;
            }

            // System.out.println(startContri);
            // System.out.println(Arrays.toString(contri));

            int ans = total;
            int count = 0;

            for (int i = 0; i < m; i++) {
                int prev = i == 0 ? startContri : contri[i - 1];
                int curr = contri[i];

                int tempTotal = total;
                tempTotal -= curr + prev;

                // now calculate for prev!
                int stPrev = i == 0 ? 1 : seller.get(i - 1);
                int nxCurr = i != (m - 1) ? seller.get(i + 1) : n + 1;

                int cookie = 1 + (((nxCurr - stPrev) - 1) / d);
                tempTotal += cookie;
