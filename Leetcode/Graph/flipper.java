...
public class flipper {
    ...
    public static void main(String[] args) {
        ...
        while (test-- > 0) {
            ...
            // otherwise find the biggest numbers and its pos in the remaining numbers
            int idx = 1;
            for (int i = 1; i < n; i++) {
                if (arr[idx] < arr[i])
                    idx = i;
            ...
            // simple now for the rest of the cases
            int befIdx = idx - 1;
            for (int i = befIdx; i >= 0; i--) {
                int l = i;
                int r = befIdx;

                // now make the arraylist for this case
                ArrayList<Integer> temp = new ArrayList<>();
                for (int j = idx; j < n; j++) {
                    temp.add(arr[j]);
                }
            ...
            // now we simply have to compare the best one possible and print it
            int best = 0;
            for (int i = 1; i < soln.size(); i++) {
                for (int j = 0; j < n; j++) {
                    int val1 = soln.get(best).get(j);
                    int val2 = soln.get(i).get(j);
                    if (val1 == val2)
                        continue;
                    if (val1 < val2) {
                        best = i;
