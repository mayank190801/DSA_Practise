...
public class brackColoring {
    ...
    public static void main(String[] args) {
        ...
        while (test-- > 0) {
            ...
            Set<Integer> one = solver(n, inp);
            Set<Integer> two = solver(n, revInp);
            ...
            if ((one != null && (one.size() == 0 || one.size() == n))
                    || (two != null && (two.size() == 0 || two.size() == n))) {
                out.println(1);
                for (int i = 0; i < n; i++) {
                    out.print(1 + " ");
                }
                ...
                if (one != null) {
                    if (one.size() == 0 || one.size() == n) {
                        ...
                        out.println(2);
                        for (int i = 0; i < n; i++) {
                            out.print((one.contains(i) ? 1 : 2) + " ");
                        }
                        out.println();
                    }
                } else {
                    if (two.size() == 0 || two.size() == n) {
                        out.println(1);
                        for (int i = 0; i < n; i++) {
                            out.print(1 + " ");
                        }
    ...
    // seperation of interests and conflicts - makes so much sense
    public static Set<Integer> solver(int n, char[] inp) {
        ...
        // if the remaining stack is not correct then it could cause this issue for sure
        boolean check = true;
        int cnt = st.size() / 2;
        while (!st.isEmpty()) {
            int idx = st.peek();
            if (st.size() > cnt && inp[idx] == ')')
                check = false;
            if (st.size() <= cnt && inp[idx] == '(')
                check = false;
            st.pop();
        }
        return !check ? null : one;
    }
