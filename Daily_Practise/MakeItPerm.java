...
public class MakeItPerm{
    ...
    public static void main(String[] args) {
        ...
        while(test --> 0){
            ...
            for(int i = 0; i < n; i++){
                arr[i] = sc.nextLong();
                if(set.contains(arr[i])) minCost += c;
                set.add(arr[i]);
            }
            long ans = Long.MAX_VALUE;
//            List<Long> list = set.stream().toList();
            List<Long> list = new ArrayList<>(set);
            for(int i = list.size() - 1; i >= 0; i--){
                long cans = 0;
                long currMax = list.get(i);
                long additions = currMax - (i + 1);
                cans += additions * d;
                cans += ((list.size() - 1) - i) * c;
                ans = Math.min(cans, ans);
            }
            //one more edge case all removed only 1 left there
            long temp = list.size() * c + d;
