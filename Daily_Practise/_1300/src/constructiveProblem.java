...
public class constructiveProblem{
    ...
    public static void main(String[] args) {
        ...
        while(test --> 0){
            ...
            //after we have value we have to figure out few edge cases and finish them right there and off
            if(val == n){
            ...
            //now some more code will be there,
            int start = map.get(val + 1).get(0);
            int end = map.get(val + 1).get(map.get(val + 1).size() - 1);
            boolean flag = true;
            for(int i = start; i <= end; i++){
                count.put(arr[i], count.get(arr[i]) - 1);
                if(arr[i] < val && count.get(arr[i]) == 0){
                    flag = false;
                    break;
                }
                arr[i] = val;
