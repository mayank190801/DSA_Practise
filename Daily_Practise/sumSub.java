...
public class sumSub{
    ...
    public static void main(String[] args) {
        ...
        //some precomp could be done he
        ArrayList<Integer>[] dp = new ArrayList[numberArrays(30) + 1];
        for(int i = 0; i < numberArrays(30) + 1; i++){
            dp[i] = new ArrayList<>();
        }

        //some where sum should also be stored for this use case
        int[] sum = new int[numberArrays(30) + 1];
        for(int i = 1; i <= 30; i++){
           int pos = numberArrays(i);
           dp[pos].add(i);
           sum[pos] = i;
        }

        //now think about it, what could be done?
        for(int i = 1; i <= numberArrays(30); i++){
            if(dp[i].size() != 0) continue;

            int minSum = Integer.MAX_VALUE;
            int idx = -1;

            //this way I could get to the fastest pos
            for(int j = 1 ; j < i; j++){
                int rem = i - j;
                int totalSum = sum[j] + sum[rem] + 1;
                if(totalSum < minSum){
                    minSum = totalSum;
                    idx = j;
                }
            ...
            //so this we know now for sure,
            //now we have the minimum sum
            sum[i] = minSum;
            dp[i].addAll(dp[idx]);
            dp[i].addAll(dp[i - idx ]);
        }
...
//        }
        ...
        while(test --> 0){
            ...
            //so now what we have to do is just print out the integers
            int[] ans = new int[n];
            Arrays.fill(ans, -1000);

            ArrayList<Integer> temp = dp[k];
            int idx = 0;
            for(int i = 0; i < temp.size(); i++){
               int times = temp.get(i);
               while(times--> 0){
                   ans[idx] = 1;
                   idx++;
               }
    ...
    public static int numberArrays(int n){
        return (n * (n + 1))/2;
    }
