...
public class minMaxSort1500{
    ...
    public static void main(String[] args) {
        ...
        while(test--> 0){
            ...
            //setting st and en in the start is tricky game for sure -- let's figure that out
            //my brain has to think a lot now, for solving such easey problem
            int st = pos[val];
            int en = n%2 == 0? pos[val + 1] : pos[val + 2];
            int lmax = st;
            int rmax = en;
            int steps = n%2 == 0? 0 : 1;
            ...
            while(true){
                ...
                //much more professional condition should be here for sure
                if( st > lmax || en < rmax) break;
                steps++;

                if(arr[st] == 1) break;
                lmax = Math.min(lmax, st);
                rmax = Math.max(rmax, en);
                st = pos[arr[st] - 1];
                en = pos[arr[en] + 1];
            }

            //using n and steps simply print out the answer for this question
            //extremely slow and rusty, since after so long, I am back at this shit
            //I will get extremely fast for sure --- no worries --- CP should be strong forever

            out.println(n%2 == 0 ? (n/2 - steps) : (n/2 + 1) - steps);
        }
