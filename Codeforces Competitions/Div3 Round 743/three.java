...
public class three{
    ...
    public static void main(String[] args) {
        ...
        while(test -- > 0){
            ...
            for(int i = 0; i < k; i++){
                arr[i] = sc.nextLong();
                count.putIfAbsent(arr[i], 0L);
                count.put(arr[i], count.get(arr[i]) + 1);
            }

            //single position pr kitne mice available??
            //ye calculate 
            List<Pair> list = new ArrayList<>();
            for(long i : count.keySet()){
                list.add(new Pair(i, count.get(i)));
            }

            Collections.sort(list, (a,b) -> Long.compare(a.a , b.a));
            ...
            for(int i = list.size()-1; i >= 0; i--){
                ...
                long currentStep = list.get(i).a;
                long number = list.get(i).b;

                if(currentStep <= cat) break;

                long stepneed = n - currentStep;

                // System.out.println(currentStep + " " + number + " " + cat + " " + stepneed );
                if((stepneed * number) <= (currentStep -cat)){
                    answer += number;
                    cat += stepneed*number;

                }else{
                    //count for that position only
                    answer += ((currentStep - cat)/stepneed);

                    if((currentStep - cat)%stepneed > 0){
                        answer += 1;
                    }
