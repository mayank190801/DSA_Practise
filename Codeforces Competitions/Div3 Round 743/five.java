...
public class five{
    ...
    public static class Graph{
        ...
        public long V;
        public ArrayList<ArrayList<Long>> edges;

        Graph(long V){
            this.V = V;
            edges = new ArrayList<>((int)V+1);
            for(int i= 0; i <= V; i++){
        ...
        public void addEdge(long from, long to){
            edges.get((int)from).add(to);
            edges.get((int)to).add(from);
        }

        //so the graph is perfect as always
        public long solve(long k){
            ...
            long[] pos = new long[(int)V+1];
            long[] visited = new long[(int)V+1];
            
            dfs(start, pos, 0, visited, false);

            long[] depthWise = new long[(int)V+1];
            for(int i= 1; i <= V; i++){
                depthWise[(int)pos[i]]++;
            }

            long[] preSum = new long[(int)V+2];
            long sum = 0;
            for(int i = 0; i < V+1; i++){
                sum += depthWise[i];
                preSum[i+1] = sum;
            ...
            long answer = V  - preSum[(int)k];
            return answer;
        }

        public long dfs(int start, long[] pos, long depth, long[] visited, boolean cond){
            
            visited[start] = 1;

            if(edges.get(start).size() == 1 && cond){
                return 1;
            }
            
            long minimum = Long.MAX_VALUE;
            for(long i : edges.get(start)){
                if(visited[(int)i] != 1){
                    long curr = dfs((int)i, pos, depth+1, visited, true);
                    minimum = Math.min(minimum, curr);
                }
            }

            pos[start] = Math.min(depth, minimum);
            return pos[start] + 1;
