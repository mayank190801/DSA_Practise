...
public class fiveUpsolve{
    ...
    public static void main(String[] args) {
        ...
        while(test -->0){
            int n = sc.nextInt();
            //k is basically the number of operations possible to be honest
            int k = sc.nextInt();
            ...
            PriorityQueue<Pair> pq = new PriorityQueue<>((a,b) -> Integer.compare(a.b, b.b));

            for(int i= 1; i <= n; i++){            
                pq.add(new Pair(i, edges.get(i).size()));
            }
            ...
            while(!pq.isEmpty()){
                ...
                if(curr.b == 1){
                    ...
                    answer.add(removed.size());
                    count += removed.size();
                    // System.out.println(removed);
                    // System.out.println(edges);
                    // System.out.println(Arrays.toString(visited));
                    //repeat the process boy, keep moving on and don't give up man
                    for(int i : removed){   
                        for(int edge : edges.get(i)){
                            edges.get(edge).remove(i);
                            if(edges.get(edge).size() == 1){
                                pq.add(new Pair(edge, edges.get(edge).size()));
                            }
                        }
                    }
