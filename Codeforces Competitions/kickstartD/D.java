...
public class D{
    ...
    public static void main(String[] args) {
        ...
        while(test --> 0){
            ...
            //now simply calcuate, the number who 
            int[] vis = new int[n + 1];
            int[] gen = new int[n+1];
            for(int i = 1; i <= n; i++){
                solver(i, graph, vis, gen);
            }
    ...
    public static void solver(int curr, ArrayList<Integer>[] graph, int[] vis, int[] gen){
        vis[curr] = 1;
        for(int edge : graph[curr]){
            if(vis[edge] == 1){
                gen[edge] = 1;
            }else{
                solver(edge, graph, vis, gen);
                gen[edge] = 1;
            }
        }
        return;
