...
public class C{
    ...
    public static void main(String[] args) {
        ...
        while(test--> 0){
            ...
            for(int i = 1; i < n; i++){
                ArrayList<Integer> next = map.get(A[i]);
                ArrayList<Integer> curr = map.get(A[i-1]);

                if(A[i] == A[i-1]) continue;
            
                for(int bef : curr){
                    for(int aft : next){
                        if(answer[aft] == -1) answer[aft] = Integer.MAX_VALUE;
                        int cget = answer[aft];
                    
                        int nscore = answer[bef] + Math.abs(bef - aft);
                        if(nscore < cget){
                            answer[aft] = Math.min(answer[aft], nscore);
                        }           
                    }
                    answer[bef] = -1;
                }
            ...
            for(int pos : map.get(A[n-1])){
                sum = Math.min(sum, answer[pos]);
            }

            out.println("Case #" + idx +  ": " + sum);
            idx++;
