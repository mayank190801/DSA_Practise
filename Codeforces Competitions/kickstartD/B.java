...
public class B{
    ...
    public static void main(String[] args) {
        ...
        while(test --> 0){
            ...
            //someprecomputation kr lete hai
            long[] startA = new long[n + 1];
            long[] endA = new long[n + 1];

            for(int i= 0; i < n; i++){
                startA[i + 1] = startA[i] + A[i];
            }

            for(int i= n-1; i >= 0; i--){
                endA[n-i] = endA[n-i-1] + A[i];
            }

            long[] startB = new long[m + 1];
            long[] endB = new long[m + 1];

            for(int i= 0; i < m; i++){
                startB[i + 1] = startB[i] + B[i];
            }

            for(int i= m-1; i >= 0; i--){
                endB[m-i] = endB[m-i-1] + B[i];
            }

            // out.println(Arrays.toString(startB));
            // out.println(Arrays.toString(endB));

            // //after this what we should do it
            long ans = 0;  //let's keep this for sure
            for(int i = 0; i <= k; i++){
                int left = i;
                int right = k-i;

                if(left > n || right > m) continue; //won't take that case


                //now solve for left > n
                long ansA = 0;
                for(int j = 0; j <= left; j++){
                    int top = j;
                    int bottom = left - j;

                    long minA = startA[top] + endA[bottom];
                    ansA = Math.max(minA, ansA);
                }
                ...
                for(int j = 0; j <= right; j++){
                    ...
                    long minB = startB[top] + endB[bottom];
                    ansB = Math.max(minB, ansB);
                }

                long total = ansA + ansB;
                ans = Math.max(total, ans);
            }

            out.println("Case #" + idx +  ": " + ans);
            idx++;
        }
