...
public class D{
    ...
    public static void main(String[] args) {
        ...
        while(test --> 0){
            ...
            ArrayList<Integer> tandemRepeats = new ArrayList<>();
            int p = 0;
            ...
            while(idx < n-1){
                if(arr[idx] == arr[idx+1]){
                    //both have moved on
                    idx+=2;
                    p+=2; //mtlb we are moving ahead of two elements now
                    tandemRepeats.add(2);
                }else{
                    //for that idx, find it's brother!!
                    int srch = idx + 1;
                    while(srch < n){
                        if(arr[idx] == arr[srch]){
                            break;
                    ...
                    // we would get the brothers of that number now
                    int len = srch - idx;
                    int index = 0;
                    for(int i = idx; i < srch; i++){
                        ans.add(new Pair(p + len + index , arr[i]));
                        index++;
                    }

                    //and then reversing the array is important tooo!!
                    reverse(arr, idx, srch-1); 
                    p += len*2;
                    tandemRepeats.add(len*2);
                    //no increment in idx
                }
            }
            ...
            for(Pair pa : ans){
                out.println(pa.a + " " + pa.b);
            }

            out.println(tandemRepeats.size());
            for(int i : tandemRepeats){
                out.print(i + " ");
            }
            out.println();
