...
public class D{
    ...
    public static void main(String[] args) {
        ...
        while(test --> 0){
            ...
            int[] arr = new int[n];
            for(int i= 0; i < n; i++){
                arr[i] = sc.nextInt();  //this is simple peace of logic for sure
            }
            ...
            while(true){
                ...
                if(l == r) break; //edge cases for prevention for sure

                // out.println(l + " " + (r-1));
                // out.println("woah");
                Triplet curr = solve(arr, n , l , r - 1);
                
                if(curr.c > ans){
                    pos = curr;
                    ans = curr.c;
                }
                //yaha answer update kro simple bois
            
                if(r == n) break;
            }

            //bejho
            out.println(pos.a + " " + pos.b);
    ...
    public static Triplet solve(int[] arr, int n, int l, int r){
        //l - r kitne 2 hai pehle ye count
        int two = 0;
        int negative = 0;
        for(int i = l ; i <= r; i++){
            if(arr[i] == 2 || arr[i] == -2) two++;
            if(arr[i] < 0) negative++;
        }

        //two ka count we have now
        if(negative % 2 == 0){
            //return the best answer simple
            return new Triplet(l, n - r - 1, two);
        }else{
            //either remove from right first negative 
            //either remove from left the first negative!! (simple no???)

            int ltwo = 0;
            int j = l;
            for(; j <= r; j++){
                if(arr[j] == 2 || arr[j] == -2) ltwo++;
                if(arr[j] < 0){
                    break;
                }
            }

            //now we have one answer for sure
            int rtwo = 0;
            int k = r;
            for(; k >= l; k--){
                if(arr[k] == 2 || arr[k] == -2) rtwo++;
                if(arr[k] < 0){
                    break;
                }                
            }

            //we have answer from both sides, return the best we could!! 
            //l r + number of two!!
            if(ltwo > rtwo){
                //return this
                // out.println(l + " " + k);
                return new Triplet(l, n-k, two - rtwo);
            }else{
                return new Triplet(j+1, n-r-1, two - ltwo);
            }
        }
    }
