...
public class three{
    ...
    public static void main(String[] args) throws IOException{
        ...
        while(test -- > 0){
            ...
            //now finding the last elemetn
            int last = list.get(list.size() -1);

            if(last < 0){
                for(int i= 0; i < list.size(); i++){
                    list.set(i, list.get(i)-(last));
                }
            ...
            // out.println(list);
            //finding the max math. abs()
            int minMax = list.get(list.size()-1);
            for(int i= 0; i < list.size()-1; i++){
                minMax = Math.max(minMax, Math.abs(list.get(i) - list.get(i+1)));
            }
