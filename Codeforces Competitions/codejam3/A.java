...
public class A{
    ...
    public static void main(String[] args) {
        ...
        while(test --> 0){
            ...
            //start and end word should combine somehow
            //just do it for n == 6 simple
            //recursion or backtracking code can be writte
            given = "---";

            for(int i= 0; i < n; i++){
                int[] vis = new int[input.length];
                vis[i] = 1;
                solve(input, i, vis, input[i], input.length - 1);
            }   
    ...
    public static void solve(String[] inp, int idx, int[] vis, String ans, int count){
        //if idx is given, simply add it for sure
        if(count == 0){
        ...
        for(int i= 0; i < inp.length; i++){
            if(vis[i] != 1){
                vis[i] = 1;
                solve(inp, i, vis, ans + inp[i], count - 1);
                vis[i] = 0;
