...
public class D{
    ...
    public static void main(String[] args) {
        ...
        //basically 1 ki positions kaha kaha hai? Simple no?
        Map<Integer, ArrayList<Integer>> map = new HashMap<>();
        for(int i =1 ; i <= 26; i++){
            map.put(i, new ArrayList<>());
        }

        //1 is always sitting at 1
        int counter = 1;
        map.get(1).add(1);

        //making with 1 simple as that
        int[] last = new int[27];
        last[1] = 1;

        //now 
        for(int i= 2; i <= n; i++){
            String outprint = "? " + 2 + " " + 1 + " " + i;
            System.out.println(outprint);

            int inp = sc.nextInt();
            if(inp > counter){  //messiah!!!
                map.get(inp).add(i);
                last[inp] = i;
                counter++;
                continue;
            ...
            //otherwise otherwise, we gotta find the impostor
            ArrayList<Pair> sorter = new ArrayList<>();
            for(int j= 1; j <= counter; j++){
                //simple
                sorter.add(new Pair(last[j], j));
            }

            Collections.sort(sorter, (a, b) -> a.a - b.a);

            //now we have some stuff for our checker to be applied no?
            int left = 0;
            int right = counter-1;
            int ans = -1;
            ...
            while(left <= right){
                ...
                //we got two pos in our hand for sure now
                int pos = sorter.get(middle).a;
                int cpos = i;
                ...
                //according to both of them make your answer!!
                int shouldbe = counter - middle;
            ...
            //after this answer would be there
            //so inactuality it's 3 for sure
            int number = sorter.get(ans).b;
        ...
        for(int i = 1; i <= counter; i++){
            String outprint = "? " + 1 + " " + map.get(i).get(0);
            System.out.println(outprint);
            
            char got = sc.nextLine().toCharArray()[0];
            mapping[i] = got;
        }
