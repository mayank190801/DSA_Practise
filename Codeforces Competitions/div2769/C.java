    ...
    public static void main(String[] args) {
        ...
        while(test --> 0){
            ...
            for(int i = second.size() -1 ; i >= 0; i--){
                if(second.get(i) == 0 && first.get(i) == 1) break;
                count++;
            }

            boolean check = true;
            //i think so weird types of checks for sure!!!
            for(int i = second.size()-1; i >= second.size() - count; i--){
                if(second.get(i) == 1 && first.get(i) == 0){
                    check = true;
                }
                if(second.get(i) == 0 && first.get(i) == 0){
                    check = false;
                }
            ...
            //find the limiting point no?
            int val = 0;
            int curr = 1;

            for(int i= 0; i < first.size() - count; i++){
                if(first.get(i) == 1){
                    val += curr;
                }
                curr = curr * 2;
            ...
            int dval = 0;
            int weird = 1;
            for(int i= 0; i < first.size() - count-1; i++){
                if(first.get(i) == 1){
                    dval += weird;
                }
                weird = weird * 2;
            ...
            int sval = 0;
            curr = 1;
            for(int i= 0; i < second.size() - count; i++){
                if(second.get(i) == 1){
                    sval += curr;
                }
                curr = curr * 2;
            ...
            int onemore = 0;
            if(check){
                // out.println(weird + " " + dval);
                onemore = weird - dval;
                if(sval > 0 || count > 1) onemore++;
            }

            // out.println(first);
            // out.println(second);
            // out.println(val + " " + sval);
            // out.println(val - sval);
            int ans = val - sval;
            if(count > 0) ans++;
