...
public class three{
    ...
    public static void main(String[] args) {
        ...
        while(test -- > 0){
            ...
            //uske niche ke available
            long[] data = new long[10005];
            for(int i= 0; i < disc.size();i++){
                int current = disc.get(i);
                int curr = current/2;
                if(current%2 == 0){
                    curr--;
                }
            ...
            for(int i = dont.size()-1; i >= 0; i--){
                int curr = dont.get(i);
                //we have to check in the below 
                if(data[curr] <= current){
                    flag = false;
                    break;
                }else{
                    current++;
