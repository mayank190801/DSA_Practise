...
public class B{
    ...
    public static void main(String[] args) {
        ...
        while(test --> 0){
            ...
            //so first find from the back bois
            int n= inp.length;
            boolean found = false;
            int index = -1;
            for(int i= n-1; i>0; i--){
                if((inp[i]-'0') + (inp[i-1]-'0') >= 10){
                    found = true;
                    index = i;
                    break;
                }
            ...
            //if not then take the first and two and them simply(that's the greedy way)
