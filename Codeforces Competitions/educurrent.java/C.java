...
public class C{
    ...
    public static void main(String[] args) {
        ...
        while(test --> 0){
            ...
            //acutally really tricky for sure man
            for(int i = n-1; i >= 0; i--){
                //piche se ek baar sbko max required krdo bas 
                long curr = h[i];
                for(int j = i-1; j >= 0; j--){
                    long now = curr - (k[i] - k[j]);
                    h[j] = max(now,h[j]); 
                }
            }
            ...
            //after doing this we have to do something else now
            //we have to figure out how to cal mana
            long energy = 0L;
            long time = 0L;
            long current = 0L;
            for(int i = 0; i < n; i++){
                if(k[i]-time >= h[i]){
                    ...
                    long before = current*(current+1L)/2L;
                    current = current + (k[i] - time);
                    long after = current*(current+1L)/2L;
