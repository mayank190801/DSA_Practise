...
public class E{
    ...
    public static void main(String[] args) {
        ...
        while(test --> 0){
            ...
            int[] par = new int[26];
            for(int i= 0; i < 26; i++){
                par[i]= i;  //everyone is themselves parent
            }

            for(int i = 0; i < n; i++){
                
                inp[i] = (char)(find(inp[i] - 'a', par) + 'a');

                while(k > 0){
                    if(inp[i] == 'a') break;                    
                    char bef = inp[i];
                    inp[i] = (char)(inp[i] - 1);  //otherwise go one level less
                    char aft = inp[i];

                    //connect them
                    int par1 = find(bef- 'a', par);
                    int par2 = find(aft -'a', par);

                    par[par1] = par2;

                    if(find(inp[i] - 'a', par) == inp[i] - 'a'){
                        //continue
                    }else{
                        //go and set it  to parent
                        inp[i] = (char)(find(inp[i] - 'a', par) + 'a');
                    }
                    k--;  //ye shayad problem kre dude
                }

                //end of the day things
                inp[i] = (char)(find(inp[i] - 'a', par) + 'a');
            }
    ...
    public static int find(int curr, int[] parent){
        if(parent[curr] == curr)
            return curr;
