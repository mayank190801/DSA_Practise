...
public class F{
    ...
    public static void main(String[] args) {
        ...
        while(test --> 0){
            ...
            for(int i= 0; i < k; i++){
                imp[i] = sc.nextInt();  //pretty important for sure bois
                left.add(imp[i]);
            }

            //n-1 edges connections for sure
            for(int i= 0; i < n-1; i++){
                int u = sc.nextInt();
                int v = sc.nextInt();

                //pretty obvious and simple for sure
                graph[u].add(v);
                graph[v].add(u);
            }

            //get an arraylist or stack whatever you want 
            Stack<Integer> stack = new Stack<>();
            ArrayList<Integer> path = new ArrayList<>();
            int[] vis = new int[n + 1];
            vis[x] = 1;
            findPath(x, y, graph, stack, vis, path);

            //so i got the path for sure
            long count = 0;
            count += path.size() - 1;

            for(int i : path){
                if(left.contains(i)) left.remove(i);
            }

            //after this add all of them in bfs gamer
            Queue<Integer> que = new ArrayDeque<>();
            vis = new int[n + 1];  //one more visited array
    ...
    //give x and an vis aray
    public static void findPath(int x, int y, ArrayList<Integer>[] graph, Stack<Integer> stack, int[] vis, ArrayList<Integer> path){
        stack.add(x);
        ...
        for(int edges  : graph[x]){
            if(vis[edges] == 0){
                vis[edges] = 1;
                findPath(edges, y, graph, stack, vis, path);
            }
        }
