    ...
    public static void main(String[] args) {
        ...
        while(test --> 0){
            ...
            for(int i = 0; i < n; i++){
                ...
                inp.add(s);
                exist.putIfAbsent(s, 0);
                exist.put(s, exist.get(s) + 1);

                if(checkPalindrome(s)) check = true;
            }
            ...
            for(int i= 0; i < n; i++){
                ...
                //simple removal baby
                exist.put(curr,exist.get(curr) - 1);
                if(exist.get(curr) == 0) exist.remove(curr);
                ...
                if(curr.length() == 2){
                    ...
                    //otherwise do something pretty cool
                    for(int j = 0; j < 26 ; j++){
                        char current = (char)('a' + j);
                        String tempo = current + temp;
                        // out.println(tempo);

                        if(exist.containsKey(tempo)){
                            check = true;
                            break;
                        }
                    }
                    ...
                    //abc
                    StringBuilder sb = new StringBuilder(curr);
                    String one = sb.reverse().substring(1,3).toString();
                    String two = sb.toString();
                    // out.println(one + " " + two);
                
                    if(exist.containsKey(one) || exist.containsKey(two)){
                        check = true;
                        break;
                    }
                }
