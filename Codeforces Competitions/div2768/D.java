...
public class D{
    ...
    public static void main(String[] args) {
        ...
        while(test --> 0){
            ...
            int min = (k-1) + (n-(k-1))/2 + 1; //(minimium finder probably)
            int r = 0;
            int l = 0;
            ...
            while(r <= n){
                ...
                //reduction
                while( l < r && curr >= min){    
                    //iss point pr condition true hogi mostly
                    if(r-l < length){
                        length = r-l;
                        la = l;
                        ra = r-1;
                    ...
                    l++;
                    if(curr < min) break;
                    //otherwise always set the answer
                }
            }


            out.println(la + " " + ra);
            //so after applying that range thingie, i am finally able to recieve this better off opportunity
            //simple as that bruh (if you know you know!!!)

            //now make cuts on the main array boi!
            int inrange = 0;
            int outrange = 0;
            int last = 0;
            ...
            for(int i = 0; i < n; i++){
                if(k == 1){  //this is k == 1 for sure
                    ans.add(new Pair(i+1,n));
                    break;
                }else{
                    //we have more 
                    if(arr[i] <= ra && arr[i] >= la) inrange++;
                    else outrange++;

                    if(inrange > outrange){
                        ans.add(new Pair(last + 1, i+1));
                        last = i+1;
                        k--;
                        inrange = 0;
                        outrange = 0;
