...
public class one{
    ...
    public static long solveTop(int[][] cases, int chosen, int required, int index, Set<Integer> set, int type){

        if(t[chosen][index] != -1) return t[chosen][index];
        ...
        for(int i= index; i < cases.length; i++){
            if(set.contains(cases[i][type])) continue;
            set.add(cases[i][type]);
            ans += solveTop(cases, chosen+1, required, i+1, set, type);
            set.remove(cases[i][type]);
        }

        return t[chosen][index] = ans;
    }

    public static long solveTop(int[][] cases, int chosen, int required, int index, Set<Integer> setone, Set<Integer> settwo){

        if(m[chosen][index] != -1) return m[chosen][index];
        ...
        long ans = 0;
        for(int i= index; i < cases.length; i++){
            if(setone.contains(cases[i][0]) || settwo.contains(cases[i][1])) continue;
            setone.add(cases[i][0]);
            settwo.add(cases[i][1]);
            ans += solveTop(cases, chosen+1, required, i+1, setone, settwo );
            setone.remove(cases[i][0]);
            settwo.remove(cases[i][1]);
        }

        return m[chosen][index] = ans;
    }
    ...
    public static void main(String[] args) {
        ...
        while(test -- > 0){
            ...
            //let's use recursion first of all
                //basic recursion
                long first = solveTop(cases, 0, 3, 0, new HashSet<>(), 0);
            
                for(int i = 0; i < 4; i++){
                    Arrays.fill(t[i], -1);
                }

                long second = solveTop(cases, 0, 3, 0, new HashSet<>(), 1);
                long third = solveTop(cases, 0, 3, 0, new HashSet<>(), new HashSet<>());
