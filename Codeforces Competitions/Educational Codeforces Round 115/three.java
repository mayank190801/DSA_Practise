...
public class three{
    ...
    public static void main(String[] args) {
        ...
        while(test -- > 0){
            ...
            double randommmm = mean * 2;
            double check = (int)randommmm;
            double random = check - randommmm;
            ...
            for(int i= 0; i <n; i++){
                long current = find - arr[i];
                if(count.containsKey(current)){
                    count.put(current, count.get(current) + counter.get(current));
                }
                count.putIfAbsent(arr[i], 0L);
                counter.putIfAbsent(arr[i], 0L);
                counter.put(arr[i], counter.get(arr[i]) + 1L);
            }
