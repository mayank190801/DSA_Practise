...
public class D{
    ...
    public static void main(String[] args) {
        ...
        long[][] data = new long[(int)n][3];
        for(int i = 0; i < n; i++){
            data[i][0] = sc.nextLong();
            data[i][1] = sc.nextLong();
            data[i][2] = sc.nextLong();
        }

        long m = sc.nextLong();
        long[][] monsters = new long[(int)m][2];
        ...
        //simple stuff no boi??
        long[] maxDamage = new long[(int)C + 1];  //with each coins what is the max damage
        
        Map<Long, Long> uniqueBest = new HashMap<>();
        for(int i = 0; i < n; i++){
            long cost = data[i][0];
            long power = data[i][1] * data[i][2];

            //idc i just want to code this problem
            uniqueBest.putIfAbsent(cost ,power);
            if(uniqueBest.get(cost) < power){
                uniqueBest.put(cost, power);  //update with best
            }
        }

        //now go through each cost and update this in maxDamage
        //later on update the maxDamage for you convenience
        for(long cost : uniqueBest.keySet()){
            long first = cost;
            long power = uniqueBest.get(cost);

            long times = 1;

            while(first <= C){
                maxDamage[(int)first] = max(maxDamage[(int)first], power);
                times++;
                power = uniqueBest.get(cost) * times;
                first = first + cost;  //simple no??
            }
        }

        // out.println(Arrays.toString(maxDamage));
        //traverse through this bad boi once
        long max = 0;
        for(int i = 1 ; i <= C; i++){
            max = max(max, maxDamage[i]);
            maxDamage[i] = max;
        }

        // out.println(Arrays.toString(maxDamage));
        //now for every coin we have best damage available no??
        //apply binary search on it bro!!!! simple
        ArrayList<Long> ans = new ArrayList<>();
        
        for(int i = 0; i < m; i++){
            long bigPower = monsters[i][0] * monsters[i][1];

            //binary search
            long coins = Long.MAX_VALUE;

            //binary search is to be applied
            boolean found = false;
            long l = 0;
            long r = C;
            while(l <= r){
                long middle = l + (r-l)/2;
                if(maxDamage[(int)middle] > bigPower){
                    coins = middle;
                    found = true;
                    r = middle - 1;
                }else{
