    ...
    public static void main(String[] args) {
        ...
        while(test -- > 0){
            ...
            for(int i= 0; i < n*m ; i++){
                ...
                dummy[i/m][i%m] = element;
            }

            //just store the points int 
            Map<Integer, ArrayList<Pair>> order = new HashMap<>();
            for(int i = 0; i < n; i++){
                for(int j = m-1 ; j >= 0; j--){
                    int current = dummy[i][j];
                    order.putIfAbsent(current, new ArrayList<>());
                    var list = order.get(current);
            ...
            for(int i= 0; i < n*m ; i++){
                ...
                int element = arr[i];
                var list = order.get(element);
                Pair req = list.get(0);
                list.remove(0);
                order.put(element, list);

                //now check the n and m
                int row = req.a;
                int col = req.b;

                for(int j = 0; j < col;  j++){
                    if(empty[row][j] == 1) count++;
                }
