...
public class four{
    ...
    public static void points(int n, int m, int num){
        xn = num/m;
        yn = num%m;
    }
    ...
    public static void main(String[] args) {
        ...
        while(test -- > 0){
            ...
            for(int i = 0; i < n*m; i++){
                int x = sc.nextInt();
                arr[i] = x;
                Pair temp = new Pair(x, i+1);
                order.add(temp);
            }

            //following sort according to the game
            Collections.sort(order, (x,y) -> Integer.compare(x.a, y.a));

            //hash\
            Map<Integer, Integer> personPos = new HashMap<>();
            int index = 1;
            for(Pair p : order){
                personPos.put(p.b, index++);
            }
            ...
            int count = 0;
            for(int i= 0; i < n*m; i++){
                int curr = personPos.get(i+1);

                if(newPos.containsKey(arr[i])){
                    curr = newPos.get(arr[i]);
                }

                int remaining = counter.get(arr[i]);
                counter.put(arr[i], counter.get(arr[i]) -1);

                int newNum = curr + remaining - 2;
                points(n, m, newNum);

                int temp = 0;

                //coud be turned into o(logn) operation
                for(int j = 0; j < yn; j++){
                    if(empty[xn][j] == 1) temp++;
                }

                empty[xn][yn] = 1;
                count += temp;

                newPos.put(arr[i], curr);

                out.println("jlkfjsdf");
