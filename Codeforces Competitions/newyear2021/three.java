...
public class three{
    ...
    public static void main(String[] args) {
        ...
        while(test --> 0){
            ...
            // //le'ts take one more test case, that is all elements should be same
            ...
            int answer = n;
            // answer = min(answer, n-max);

            for(int i= 0; i < n-1; i++){
                for(int j= i+1; j < n; j++){
                    //now select two element values and the difference between them
                    if(i == j) continue;
                    double l = arr[i];
                    double r = arr[j];
    ...
    public static int check(int[] arr,double first, double d){
        int count = 0;
        for(int i = 0; i < arr.length ;i++){
            if(abs(((double)arr[i])-first) > eps) count++; 
            //you will learn from 
            //Important shit for sure
            first += d;
        }
