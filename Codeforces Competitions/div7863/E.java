...
public class E{
    ...
    public static void main(String[] args) {
        ...
        //code here
            ...
            int n = sc.nextInt();
            long[] arr = new long[n];
            long[] fmoves = new long[n];
            for(int i= 0; i < n; i++){
                arr[i] = sc.nextLong();  //simple sober technique bois
                if(arr[i] % 2 == 0){
                    fmoves[i] = arr[i]/2;
                }else{
                    fmoves[i] = arr[i]/2 + 1;
            ...
            //now 
            segmentTree seg = new segmentTree(fmoves);
            seg.buildTree(0, n-1, 1);  //simply builde
        
            long ans = Long.MAX_VALUE;

            for(int i = 1; i < n - 1; i++){
                //first do for one before and one after simply
                long moves = arr[i] + arr[i -1];
                moves = moves/3;
                if((arr[i] + arr[i-1]) % 3 != 0)  moves++;

                ans = min(ans, moves);
                //then for the rest  long moves = arr[i-1];
                moves = arr[i] + arr[i+1];
                moves = moves/3;
                if((arr[i] + arr[i + 1]) % 3 != 0)  moves++;
                ...
                long min = min(seg.query(0, n-1, 0, i-1, 1), seg.query(0, n-1, i+1, n-1 ,1));
                min += arr[i]/2;
                if(arr[i] %2 ==1 ) min++;
                ans = min(ans, min);
            }
            
            long moves = arr[0] + arr[1];
            moves = moves/3;
            if((arr[0] + arr[1]) % 3 != 0)  moves++;

            ans = min(ans, moves);
           

            //but after that what now?
            long min = seg.query(0, n-1, 1, n-1 ,1);
            // out.println(min);
            min += arr[0]/2;
            if(arr[0] %2 ==1 ) min++;
           ...
           //then for the rest  long moves = arr[i-1];
           moves = arr[n-1] + arr[n-2];
           moves = moves/3;
           if((arr[n-1] + arr[n-2]) % 3 != 0)  moves++;

           ans = min(ans, moves);
            
            //but after that what now?
            min = seg.query(0, n-1, 0, n-2 ,1);
            min += arr[n-1]/2;
            if(arr[n-1] %2 ==1 ) min++;
            ans = min(ans, min);


            //check for the case where breaking middle guy breaks both on sides!! 
            //pretty cool case for sure
            for(int i = 1; i < n-1; i++){
                min = max(arr[i-1], arr[i+1]);
                ans = min(min, ans);
            }
