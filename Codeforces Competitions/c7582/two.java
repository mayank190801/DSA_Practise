...
public class two{
    ...
    public static void main(String[] args) {
        ...
        //code below
        //let's first check the solution
        int test = sc.nextInt();
        while(test --> 0){
            int n = sc.nextInt();
            int a = sc.nextInt();
            ...
            //let's check the max values of a & b
            int[] ans = new int[n];            
            if(a > b){
                int curr = n;
                for(int i= 0; i < n; i++){
            ...
            for(int i = 1; i < n-1; i++){
                if(ans[i] > ans[i+1] && ans[i] > ans[i-1]) maxima++;
                if(ans[i] < ans[i+1] && ans[i] < ans[i-1]) minima++;
            }

            if(maxima == a && minima == b){
                for(int i : ans){
                    out.print( i + " ");
                }
                out.println();    
