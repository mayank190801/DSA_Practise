...
public class three{
    ...
    public static void main(String[] args) {
        ...
        while(test -- > 0){
            ...
            int[] one = new int[n];
            int[] two = new int[n];
            ArrayList<Pair> onee = new ArrayList<>();
            ArrayList<Pair> twoo = new ArrayList<>();
            Set<Integer> winner = new HashSet<>();

            for(int i = 0; i < n; i++){
                one[i] = sc.nextInt();
                onee.add(new Pair(i+1, one[i]));
            }
            for(int i = 0; i < n; i++){
                two[i] = sc.nextInt();
                twoo.add(new Pair(i+1, two[i]));
            } 

            Collections.sort(onee, (a,b) -> Integer.compare(b.b, a.b));
            Collections.sort(twoo, (a,b) -> Integer.compare(b.b, a.b));

            Map<Integer, Integer> map1 = new HashMap<>();
            Map<Integer, Integer> map2 = new HashMap<>();

            for(int i = 0; i < n; i++){
                map1.put(onee.get(i).a, i);
                map2.put(twoo.get(i).a, i);
            }

            //keep pushing both sides until you face stopage while switching
            int upar = 0;
            int niche = 0;

            winner.add(onee.get(0).a);
            winner.add(twoo.get(0).a);
            upar = map1.get(twoo.get(0).a);
            niche = map2.get(onee.get(0).a);
            ...
            while(true){
                ...
                for(; l < upar; l++){
                    count++;
                    int curr = onee.get(l).a;
                    if(winner.contains(curr)) continue;
                    //otherwise
                    winner.add(curr);
                    niche = max(map2.get(curr), niche);
                }

                for(; r < niche; r++){
                    count++;
                    int curr = twoo.get(r).a;
                    if(winner.contains(twoo.get(r).a)) continue;

                    winner.add(curr);
                    upar = max(map1.get(curr), upar);
                }
                if(count == 0) break;
            }
            ...
            for(int i = 0; i < n;i++){
                answer[i] = winner.contains(i+1) ? 1 : 0;
            }
