...
public class C{
    ...
    public static void main(String[] args) {
        ...
        while(test --> 0){
            ...
            //now solve for each step boi
            for(int i = 0; i < N; i++){
                long steps = solve(N, D, i, arr);
                // out.println(steps);
                ans = min(ans, steps);
            }
    ...
    public static long solve(long N, long D, int idx, long[] arr){

        //go any i guess
        int l = idx;
        int r = idx;
        long rotations = 0;
        while(l >= 0 && arr[l] == arr[idx]){
            l--;
        }

        while(r < arr.length && arr[r] == arr[idx]){
            r++;
        }
  

        long curr = arr[idx];
        //now we have the ranage
        while(true){
            //int go left or right?
            if(l >= 0 && r < arr.length){
                long left = arr[l];
                long right = arr[r];

                long expansionLeft =  -1;
                long expanionRight = -1;
                if(left < curr){
                    expansionLeft = min(abs(curr - left), left + D - curr);
                }else{
                    expansionLeft = min(abs(curr - left), curr + D - left);
                }
                // long expansionleft = min(abs(left - curr), );
                //figure this out first!! 
                if(right < curr){
                    expanionRight = min(abs(curr - right), right + D - curr);
                }else{
                    expanionRight = min(abs(curr - right), curr + D - right);
                }


                //go aage 
                if(expansionLeft > expanionRight){
                    rotations += expanionRight;
                    curr = right;
                    //expand now
        ...
        //last case is making sure everyone is zero
        if(curr != 0){
            long expansionLeft =  -1;
            // long expanionRight = -1;
            if(0 < curr){
                    expansionLeft = min(abs(curr - 0), 0 + D - curr);
            }else{
                    expansionLeft = min(abs(curr - 0), curr + D - 0);
            }
