...
public class D{
    ...
    public static void main(String[] args) {
        ...
        //code here
        //only coding part is left here! (simple)
        int test = sc.nextInt();
        while(test --> 0){
            int r = sc.nextInt();
            int c = sc.nextInt();
            ...
            //i gotta make the big table for this for sure!
            int[][] vis = new int[2*r][2*c];
            char[][] original = new char[2*r][2*c];
            for(int i= 0; i < r; i++){
                for(int j = 0; j < c; j++){
                    for(int k = 2 * i; k < 2* i + 2; k++){
                        for(int l = 2 * j; l < 2 * j + 2; l++){
                            original[k][l] = table[i][j];    
                            if(table[i][j] == '*'){
                                vis[k][l] = 2;
                            }
                        }
            ...
            //figure out exact keymapping bois
            //after this it simple try going odwn

            //now we are right most
            //trying going down with right as proirity
            int cx = 0;
            int cy = 2*c - 1;

            //keep appending something for god sake!
            while(true){
                ///if right possible go right
                if(cx >= 2*r - 1) break;

                System.out.println(cx + " " + cy); 

                //go right 
                if(cy < (2*c - 1) && vis[cx][cy + 1] == 0){
                    vis[cx][cy] = 1;
                    cy++;
                    sb.append("E");
                    continue;
                ...
                //otherwise go left simple
                sb.append("W");
                vis[cx][cy] = 1;    
                cy--;
            }

            //print sb and visited till here now!! (simple)
            for(int[] vi : vis){
                for(int i : vi){
                    out.print(i);
                }
