...
public class E{
    ...
    public static void main(String[] args) {
        ...
        while(test --> 0){
            ...
            for(int i= 0; i < k; i++){
                int x = sc.nextInt();
                pos[x] = 0; //simply mark them zero no?
            }
    ...
    public static class Graph{
        ...
        //keep adding functions in here!!
        //pretty long and clear mind question to be honest for me!! (pretty annoying as well)
        public void setPos(int[] pos){
            int[] visited = new int[V];
            helper(1, visited, pos, 0);
            out.println(Arrays.toString(pos));
        }

        public int helper(int curr, int[] visited, int[] pos, int parent){
            //minimum from child + 1s
            int min = Integer.MAX_VALUE-1;
            for(int edge : edges.get(curr)){
                if(edge != parent){
                    min = min(pos[curr],helper(edge, visited, pos, curr)+1);
                }
            }
            return pos[curr] = min(pos[curr],min+1); 
        }
