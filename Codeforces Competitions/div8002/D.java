...
public class D{
    ...
    public static void main(String[] args) {
        ...
        while(test --> 0){
            ...
            //after this grpah is ready simpel
            int[] ans = new int[1];  //how many operations minimum
            int[] vis = new int[n + 1];
            
            solve(1, graph, ans, range, vis);
            out.println(ans[0]);
    ...
    public static long solve(int curr, ArrayList<Integer>[] graph, int[] ans, long[][] range, int[] vis){
        vis[curr] = 1;
        // out.println(curr);
        ...
        for(int edge : graph[curr]){
            if(vis[edge] != 1){
                sum += solve(edge, graph, ans, range, vis);
            }
        }

        //now simply take power from sum and solve this question!
        // out.println(sum + " " + curr + " " + range[curr-1][0]);
        if(sum < range[curr-1][0]){
            ans[0]++;    
            return range[curr-1][1];
        }else{
            return min(sum, range[curr-1][1]);
        }
    }
