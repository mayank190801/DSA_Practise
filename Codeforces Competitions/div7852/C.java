...
public class C{
    ...
    public static void main(String[] args) {
        ...
        //code here
        ArrayList<Integer> palindromic = new ArrayList<>();
        for(int i = 1; i <= 40_000; i++){  
            String parse = String.valueOf(i);
            int l = 0;
            int r = parse.length()  - 1;
            boolean flag = true;
            while(l <= r){
                if(parse.charAt(l) != parse.charAt(r)) flag = false;
                l++;
                r--;
            }

            if(flag) palindromic.add(i);
        }

        long[] dp = new long[400001];  //simple logix
        dp[0] = 1;
        for(int i= 1; i < dp.length; i++){
            for(int j= 0; j < palindromic.size(); j++){
                if(palindromic.get(j) > i) break;
                //simple logix
                int get = palindromic.get(j);
        ...
        // }
        //only 500 elements for sure!! (pretty let's go move baby)
