...
public class B{
    ...
    public static void main(String[] args) {
        ...
        //code here
        int test = sc.nextInt();  //fuck this question
        while(test --> 0){
            String get = sc.nextLine();
            char[] inp = get.toCharArray();

            int n = inp.length;
            boolean flag = true;
            //i did a silly mistake for sure
            Set<Character> set = new HashSet<>();
            for(int i= 0; i < n; i++){
                if(set.contains(inp[i])) break;
                set.add(inp[i]);
            }

            //now add the remaining part in the question for sure! (simple logix)
            int distinct = set.size();

            //now i know the req substring - simple please     
            //check one more 
            String temp = "";
            for(int i= 0; i < n/set.size(); i++){
                temp += get.substring(0, set.size());
            }

            //after this - add remaining part too
            int x = n%distinct;
            temp += get.substring(0, x);

            if(temp.compareTo(get) != 0){
                flag = false;
            }
