...
public class C{
    ...
    public static void main(String[] args) {
        ...
        //simple
        //if more than two zeros on both, then not possile for sure
        int[] rows = new int[n];
        int[] cols = new int[n];

        long[] checkerRows = new long[n];
        segmentTree segRows = new segmentTree(checkerRows);
        
        long[] checkerCols = new long[n];
        segmentTree segCols = new segmentTree(checkerCols);


        //all simple zero now issues in that
        for(int i = 0; i < q; i++){
            int t = sc.nextInt();
            if(t == 1){
                int x = sc.nextInt();
                ...
                segRows.updateRange(0, n-1, x, x, 1, 1);
                segCols.updateRange(0, n-1, y, y, 1, 1);
                ...
                //get the val
                long val1 = segRows.query(0, n-1, x1, y1, 1);
                long val2 = segCols.query(0, n-1, x2, y2, 1);
