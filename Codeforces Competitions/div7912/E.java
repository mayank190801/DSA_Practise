...
public class E{
    ...
    public static void main(String[] args) {
        ...
        while(test --> 0){
            ...
            //simple two pointer would work for sure bro!
            int one = 0;
            int zero =  0;
            ...
            // boolean get = check(3, inp, one, zero);
            // System.out.println(get);
    
            //max answer for now
            int ans = min(zero, one);  //worst case answer simple
            int left = 0;
            int right = ans-1;  //isse kam pr check kro simple

            while(left <= right){
                int middle = left + (right - left)/2;
                if(check(middle, inp, one, zero)){
                    ans = middle;
                    right = middle - 1;
                }else{
                    left = middle + 1;
    ...
    public static boolean check(int middle, char[] inp, int tone, int tzero){
        ...
        //writing these functions is usally hard for sure
        while(lidx < n){  //maximum one you can take from left, simple
            if(inp[lidx] == '1'){
                leftOne++;
            }else{  
                leftZero++;
        ...
        //right se bhi aage bda lo boss no worries
        while(ridx >= 0){
            if(inp[ridx] == '1') break;
            ridx--;
            rightZero++;
        ...
        //just check for now once again for sure
        int maxZeroLeft = tzero - (leftZero + rightZero);
        int maxOneRemoved = leftOne + rightOne;

        if(max(maxZeroLeft, maxOneRemoved) <= middle) return true;

        //next time se remove one and right se increase one
        for(int i = 0; i < middle; i++){
            //simple remove one , and go left
            while(lidx >= 0){
                if(inp[lidx] == '1') {
                    leftOne--;
                    lidx--;
            ...
            maxZeroLeft = tzero - (leftZero + rightZero);
            maxOneRemoved = leftOne + rightOne;
