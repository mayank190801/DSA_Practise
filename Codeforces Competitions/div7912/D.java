...
public class D{
    ...
    public static void main(String[] args) {
        ...
        //simple sober techniques
        for(int i = 0; i < m; i++){
            int u = sc.nextInt();
            int v = sc.nextInt();
            graph[u].add(v);  //single directional edge for sure bois
        }

        //now what, it's simple for sure
        //binary search 
        long left = 0;
        long right = random;    
        long ans = Long.MAX_VALUE;  //check if the answer could be made or not, simple
