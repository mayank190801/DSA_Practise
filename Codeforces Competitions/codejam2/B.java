...
public class B{
    ...
    public static void main(String[] args) {
        ...
        while(test --> 0){
            ...
            //sorting arrays are important
            long[][] inp = new long[n][p];
            ...
            //after this we have out input
            //sort it simply
            for(int i = 0; i < n; i++){
                sort(inp[i]);
            }

            long[][] minMax = new long[n + 1][2];
            for(int i= 0; i < n; i++){
                minMax[i + 1][0] = inp[i][0];
                minMax[i + 1][1] = inp[i][p-1];
            }

            // out.println(Arrays.deepToString(minMax));
            long[][] dp = new long[n + 1][2];
            for(int i= 1; i <= n; i++){
                Arrays.fill(dp[i], Long.MAX_VALUE);
            }

            //for this we gotta do sometings
            for(int i= 0; i < n; i++){
                //how to write code for this bad boi huh? - think about it brother
                long first = minMax[i][0];

                long lower = minMax[i + 1][0];
                long higher = minMax[i + 1][1];
                if(first < minMax[i + 1][0]){
                    long before = higher - first + higher - lower;
                    long after  = higher - first;

                    dp[i + 1][0] = min(dp[i + 1][0], before + dp[i][0]);
                    dp[i + 1][1] = min(dp[i + 1][1], after + dp[i][0]);
                }else if(first > minMax[i + 1][1]){
                    long before = first - lower;
                    long after  = first - lower + higher - lower;
                ...
                long second = minMax[i][1];
                
                if(second < minMax[i + 1][0]){
                    long before = higher - second + higher - lower;
                    long after  = higher - second;

                    dp[i + 1][0] = min(dp[i + 1][0], before + dp[i][1]);
                    dp[i + 1][1] = min(dp[i + 1][1], after + dp[i][1]);
                }else if(second > minMax[i + 1][1]){
                    long before = second - lower;
                    long after  = second - lower + higher - lower;
            ...
            //we would have our answer at the end
            // out.println(Arrays.deepToString(dp));
            long minimum = min(dp[n][0], dp[n][1]);
            // out.println(minimum);
            out.println("Case #" + cases + ": " + minimum);
            cases++;
