...
public class D{
    ...
    public static void main(String[] args) {
        ...
        while(test --> 0){
            ...
            //after this it is pretty obvious for sure! (think about it brother for once) s
            //make a 2D array for solving 
            long[][][] dp = new long[n][n][2];
            
            //solve this please! (you have practised these question before in pepcoding)
            //think think think think think think
            //traverse with  your creativity
            for(int gap = 1; gap < n; gap++){
                int si = 0;
                int ei = gap;

                while(ei < n){  //keep solving now

                    //solve for this guy simple
                    //solve for left 
                    long temp = dp[si + 1][ei][0] + min(abs(arr[si + 1] - arr[si]), min(abs(0 - arr[si]) + abs(D - arr[si + 1]), abs(D - arr[si]) + abs(0 - arr[si + 1])));
                    long tempo = dp[si + 1][ei][1] + min(abs(arr[ei] - arr[si]), min(abs(0 - arr[si]) + abs(D - arr[ei]), abs(D - arr[si]) + abs(0 - arr[ei])));
        
                    //i am fucking confused now
                    dp[si][ei][0] = min(temp, tempo);

                    //solve for ending at right
                    temp = dp[si][ei - 1][0] + min(abs(arr[si] - arr[ei]), min(abs(0 - arr[si]) + abs(D - arr[ei]), abs(D - arr[si]) + abs(0 - arr[ei])));
                    tempo = dp[si][ei - 1][1] + min(abs(arr[ei] - arr[ei - 1]), min(abs(0 - arr[ei - 1]) + abs(D - arr[ei]), abs(D - arr[ei - 1]) + abs(0 - arr[ei])));
        
                    dp[si][ei][1] = min(temp, tempo);
            ...
            //simply print the minimum of the answer
            //how to print a 3D array boss?


            long ans = min(dp[0][n-1][0] + min(abs(0 - arr[0]), abs(D - arr[0])), dp[0][n-1][1] + min(abs(0 - arr[n-1]), abs(D - arr[n-1])));
            //convert them to zero too no?? - who will do that brother??
            //you gotta do that only
