...
public class four{
    ...
    public static class Graph{
        ...
        public String solve(long xor, long k, long[] arr){
            int[] visited = new int[V+1];
            //we will start from node 1 heheh
         
            dfs(1,visited,xor, arr);
            if(count.size() == 0){
                return "NO";
            }
...
// /            System.out.println(edges);
        ...
        public long dfs(int curr, int[] visited, long xor, long[] arr){
            visited[curr] =1;
            long tempXor = 0;
            ...
            for(int i : edges.get(curr)){
                if(visited[i] != 1){
                    long val = dfs(i, visited, xor, arr);
                    ...
                    if(val == xor){
                        //so i am able to remove with no problem
                        edges.get(curr).remove(edges.get(curr).indexOf(i));
                        edges.get(i).remove(edges.get(i).indexOf(curr));
                        count.add(i);
                        count.add(curr);
                        return -1;
                    }else{
                        //we gotta add this in the tempxor value
                        tempXor ^= val;
                    }
    ...
    public static void main(String[] args) {
        ...
        //let's have a new beginning for this question
        int test = sc.nextInt();
        while(test -- > 0){
