...
public class C{
    ...
    public static void main(String[] args) {
        ...
        while(test --> 0){
            ...
            //now suppose think about it for once! hmm
            for(int i = 0; i < n - 1; i++){
                int from = sc.nextInt();
                int two = sc.nextInt();
            ...
            //now calculate height for each vertex no?
            int[] members = new int[n + 1];
            int[] vis = new int[n + 1];
            
            int[] low = new int[2];
            low[0] = Integer.MAX_VALUE;
            counter(1, vis, graph, members, low, 0);
            int ans = low[0];
            boolean check = low[1] == 0 ? true : false;

            // out.println(ans);
            // out.println(check);
            
            int answer = n - (ans + 1 + ans);
            if(!check) answer--;
            out.println(answer);
    ...
    public static void counter(int curr, int[] vis, ArrayList<Integer>[] graph, int[] members, int[] low, int height){
        vis[curr] = 1;

        if(graph[curr].size() <= 2 && curr != 1){
            if(height <= low[0]){
                if(height < low[0]){
                    low[0] = height;
                    low[1] = graph[curr].size() == 1 ? 0 : 1;  
                }else{
                    int get = graph[curr].size() == 1 ? 0 : 1;  
                    low[1] = min(get, low[1]);
                }
            }
            low[0] = min(height, low[0]);
            return;
        }

        for(int edge : graph[curr]){
            if(vis[edge] != 1){
                counter(edge, vis, graph, members,low, height + 1);
            }
        }
