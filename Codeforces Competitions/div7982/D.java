...
public class D{
    ...
    public static void main(String[] args) {
        ...
        while(test --> 0){
            ...
            for(int i = 0; i < n; i++){
                //this is pretty simple and obvious
                inp[i] = sc.nextLine().toCharArray();
            }

            //replicate this four times simple
            int[] verti = new int[m];
            for(int j = 0; j < m; j++){
                int cnt = 0;
                for(int i = 0; i < n; i++){
            ...
            int[] leftRightVert = new int[m];
            int radd = verti[0];  //think about this for sure
            for(int j= 1; j < m; j++){
                leftRightVert[j] = radd + leftRightVert[j-1];
                radd += verti[j];
            }

            int[] rightLeftVert = new int[m];
            radd = verti[m-1];  //think about this for sure
            for(int j= m-2; j >= 0; j--){
                rightLeftVert[j] = radd + rightLeftVert[j+1];
                radd += verti[j];
            }
            
            // System.out.println(Arrays.toString(leftRightVert));
            // System.out.println(Arrays.toString(rightLeftVert));

            int[] leftRightHori = new int[n];
            radd = hori[0];  //think about this for sure
            for(int j= 1; j < n; j++){
                leftRightHori[j] = radd + leftRightHori[j-1];
                radd += hori[j];
            }

            int[] rightLeftHori = new int[n];
            radd = hori[n-1];  //think about this for sure
            for(int j= n-2; j >= 0; j--){
            rightLeftHori[j] = radd + rightLeftHori[j+1];
                radd += hori[j];
            }
            ...
            System.out.println(Arrays.toString(leftRightVert));
            System.out.println(Arrays.toString(rightLeftVert));
            System.out.println(Arrays.toString(leftRightHori));
            System.out.println(Arrays.toString(rightLeftHori));

            int ans = Integer.MAX_VALUE;
            Pair p = new Pair(-1, -1);
            for(int i= 0; i < n; i++){
                for(int j = 0; j < m; j++){
                    int left = leftRightVert[j];
                    int right = rightLeftVert[j];
                    int up = leftRightHori[i];
                    int down =  rightLeftHori[i];
