...
public class D{
    ...
    public static void main(String[] args) {
        ...
        while(test--> 0){
            ...
            char[] inp = s.toCharArray();  //simple stuff

            //now

            //indices of all 0 le lo simple bas
            ...
            if(pos.size() == 1){
                ...
                if(n%2 == 0){
                    ...
                    out.println("YES");
                    for(int i= 0; i < n; i++){
                        if(i == pos.get(0)) continue;
                        out.println((pos.get(0) + 1) + " " + (i + 1)); 
                    }
                }
                continue;
            }
            ...
            //otherwise simple question solve is there
            //first ke liye odd, last ke liye odd, baaki sabke liye even
            ArrayList<Pair> edges = new ArrayList<>();

            int last = n-1;
            while(s.charAt(last) == '1'){
                last--;
            }

            String sn = s.substring(last + 1, n) + s.substring(0, last + 1);
            inp = sn.toCharArray();
            ...
            int idx = pos.get(0);
            //write down code for two cases simple

            boolean flag1 = true; //temp
            if(idx > 0){
                edges.add(new Pair(idx + 1, idx));

                //now solve the answer for the rest
                int pt = idx + 1;  //pointing a bit ahead
                for(int j = 1; j < pos.size() - 1; j++){
                    int get = pos.get(j);
                    int ahead = pos.get(j + 1);

                    //middle element should be even
                    int counter = 0;
                    while(pt < get){
                        edges.add(new Pair(pt + 1, get + 1));
                        counter++;
                        pt++;
                    }
                    ...
                    if(counter % 2 == 1){
                        if(ahead - get == 1){
                            ...
                            edges.add(new Pair(get + 1, get + 2));
                            pt = get + 2;
                        }
                    }else{
                        pt = get + 1;
                ...
                //after this above process is done!!
                //add all remaining ones to that bad boi
                if(flag1 == false){
                    //dont' do shit
                }else{
                    int boom = pos.get(pos.size() - 1);
                    int counter = 0;
                    while(pt < boom){
                        edges.add(new Pair(pt + 1, boom + 1));
                        counter++;
                        pt++;
                    }

                    //add from the starting also please
                    for(int i = 0; i < pos.get(0) - 1; i++){
                        edges.add(new Pair(i + 1, boom + 1));
                        counter++;
                    }
            ...
            if(flag1 == false){
                int sidx = pos.get(1);
                //write down code for this also
                if(sidx - idx <= 1){
                    flag2 = false;
                }else{
                    edges.add(new Pair(idx + 1, idx + 2));

                    //now solve the answer for the rest
                    int pt = idx + 2;  //pointing a bit ahead
                    for(int j = 1; j < pos.size() - 1; j++){
                        int get = pos.get(j);
                        int ahead = pos.get(j + 1);
            ...
            if(flag1 || flag2){
                for(int i = 0; i < pos.size() - 1; i++){
                    edges.add(new Pair((pos.get(i) + 1),(pos.get(i + 1) + 1)));
                }
