...
public class C{
    ...
    public static void main(String[] args) {
        ...
        long[] pointer = new long[n];
        long[] speed = new long[n];

        for(int i = 0; i < n; i++) pointer[i] = sc.nextLong();
        ...
        for(int i = 0; i < n; i++){
            if(i == n-1){
                max += speed[i] * (l - pointer[i]);
            }else{
                max += speed[i] * (pointer[i+1] - pointer[i]);
            }
        }
        ...
        boolean pick = check(0L, pointer, speed, l);
    ...
    public static boolean check(long middle, long[] pointer, long[] speed, long l, long time, int k){
        int n= pointer.length;
        PriorityQueue<Pair> pq = new PriorityQueue<>((a,b) -> Long.compare(b.b,a.b));
        ...
        //now for each value put in there
        for(int i= 1; i < n; i++){
            long current = 0;
            long removal = 0;
            if(i == n-1){
                current = speed[i] * (l - pointer[i]);
                removal = speed[i-1] * (l - pointer[i]);
            }else{
                current = speed[i] * (pointer[i+1] - pointer[i]);
                removal = speed[i-1] * (pointer[i+1] - pointer[i]);
            }

            //always add
            pq.add(new Pair(i, current - removal));
        }

        while(time > middle){
            if(k == 0) break;
            if(pq.isEmpty()) return false;
            if(pq.peek().b <= 0) return false;
            k--;
            //ni toh
            Pair curr = pq.poll();
            time -= curr.b;

            int get = set.higher((int)curr.a);
            set.remove(curr.a);
            ...
            //update aage vaali value, lekin kaise
