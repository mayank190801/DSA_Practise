...
public class F{
    ...
    public static void main(String[] args) {
        ...
        while(test--> 0){
            int n = sc.nextInt();
            char[][] inp = new char[2][n];
            for(int i = 0; i < 2; i++){
                inp[i] = sc.nextLine().toCharArray();
            } 

            //after this i have the input for sure
            int ans = Integer.MAX_VALUE;

            int llast = -1;
            int rlast = -1;

            //some babies to help me for sure
            //pretty useful for sure boi
            int rTopCount = 0;
            int rBottomCount = 0;
            int lTopCount = 0;
            int lBottomCount = 0;

            //setting some prereq for sure
            for(int i= 0; i < n; i++){
                if(inp[0][i] == '*'){
                    rTopCount++;
                    rlast = i;
            ...
            //now simply come at some position and do this bachcha
            for(int i= 0; i < n; i++){
                ...
                //main logic of this problem comes in here
                int leftMin = 0;

                boolean lflagUp = true;
                if(llast != -1){
                    int aboveCost = lBottomCount + abs(llast - i);
                    int belowCost = lTopCount + abs(llast - i);

                    if(aboveCost <= belowCost){
                        lflagUp = true;
                    }else{
                        lflagUp = false;
                    }

                    leftMin = min(aboveCost, belowCost);
                }

                int rightMin = 0;
                boolean rflagUp = true;
                if(rlast != -1){
                    int aboveCost = rBottomCount + abs(rlast - i);
                    int belowCost = rTopCount + abs(rlast - i);
                    ...
                    rightMin = min(aboveCost, belowCost);
                }

                //just calculate your answer at this point simple
                int total = leftMin + rightMin;
                if(!lflagUp || !rflagUp || inp[1][i] == '*') total++;
                ans = min(total, ans);

                // out.println(total + " 1");

                //update the constraints baby
                //left side iske add krde current situation
                //update lidx if not done yet, simple
                if(inp[0][i] == '*'){
                    lTopCount++;
                    if(llast == -1) llast = i;
                }
                ...
                //updated lest side for sure
            }
            
            //do the same for bottom array no worries
            //do it for bottom row now boi
            llast = -1;
            rlast = -1;
            ...
            for(int i= 0; i < n; i++){
                ...
                //just calculate your answer at this point simple
                int total = leftMin + rightMin;
                if(!lflagDown || !rflagDown || inp[0][i] == '*') total++;
                ans = min(total, ans);
