...
public class C{
    ...
    public static void main(String[] args) {
        ...
        while(test --> 0){
            ...
            int[] arr = new int[n];
            for(int i= 0; i < n; i++){
                arr[i] = sc.nextInt(); ///pretty simple
            }

            //now count from the end
            if(arr[0]%2  == arr[n-1]%2){
                //don't jump uselessly
                int ans = 0;
                for(int i= 1; i < n; i++){
                    if(arr[i]%2 == arr[0]%2){
                        ans++;
                ...
                //if different parity, we gotta shift, and that, gott amake some dope
                int[] dp = new int[n];
                dp[n-1] = 1;
                for(int i = n-2; i >= 0; i--){
                    dp[i] = dp[i + 1] + (arr[i] % 2 == arr[n-1] % 2 ? 1 : 0);
                }
                // System.out.println(Arrays.toString(dp));
                
                int ans = Integer.MAX_VALUE;  //we gotta shift no matter what
                //now go from the start 
                int cnt = 0;
                for(int i = 0; i < n; i++){
                    if(arr[0] % 2 != arr[i] % 2) continue;
