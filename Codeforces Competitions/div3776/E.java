...
public class E{
    ...
    public static void main(String[] args) {
        ...
        while(test --> 0){
            ...
            //simply the dates at which paper would be held
            int[] arr = new int[n];
            for(int i= 0; i < n; i++){
                arr[i] = sc.nextInt();
            }
            ...
            TreeMap<Integer, Integer> minDays = new TreeMap<>();
            //one more treeMap is required to store the biggest possible space for this question
            TreeMap<Integer , Set<Integer>> biggestGap = new TreeMap<>();

            int curr = 1;
            for(int i= 0; i < n; i++){
                int restDays = arr[i] - curr;
                curr = arr[i] + 1;

                //simple, now edit mindays
                minDays.putIfAbsent(restDays, 0);
                minDays.put(restDays, minDays.get(restDays) + 1);

                //updating biggest Gap for this question
                biggestGap.putIfAbsent(restDays, new HashSet<>());
                biggestGap.get(restDays).add(i);  //the index which lives in here for free
            ...
            int minAns = minDays.firstKey(); //so this is the current answer (without moving any day)

            //firstly being abstract, and ignoring edge cases
            for(int i = 0; i < n-1; i++){
                //current and nextDay ko update krdo bas
                //fir check maar lena boss
                int first = arr[i];
                int second = arr[i+1];
                int before = i == 0 ? 1 : arr[i-1] + 1;  //simple no?
                
                int restDaysToday = first - before;
                int restDaysTom = second - first - 1;

                // System.out.println(restDaysToday);
                // System.out.println(restDaysTom);

                //update both of these from two places simple
                minDays.put(restDaysToday, minDays.get(restDaysToday) - 1);
            
                //simple updation
                if(minDays.get(restDaysToday) == 0){
                    minDays.remove(restDaysToday);
                }
                
                minDays.put(restDaysTom, minDays.get(restDaysTom) - 1);
                //simple updation
                if(minDays.get(restDaysTom) == 0){
                    minDays.remove(restDaysTom);
                }

                //remove from biggest Gap too!! bois!! hustle is real for this code man!!
                //my code is too lenghty too work for some reasons
                biggestGap.get(restDaysToday).remove(i);
                
                if(biggestGap.get(restDaysToday).size() == 0){
                    biggestGap.remove(restDaysToday);
                }

                biggestGap.get(restDaysTom).remove(i+1);
                if(biggestGap.get(restDaysTom).size() == 0){
                    biggestGap.remove(restDaysTom);
                }


                //ALL TIME PASS UPDATIONS HAVE BEEN DONE FOR SURE
                //NOW COME SERIOUS STUFF
                //If i put it in biggest space possible, then what??
                //add biggest space in both the spaces, then pick up and update the answer
                //then bring back everything to original spacing baby!!! (think about this)

                int nNewDays = second - before;
                minDays.putIfAbsent(nNewDays, 0);
                minDays.put(nNewDays, minDays.get(nNewDays) + 1);

                //updating biggest Gap for this question
                biggestGap.putIfAbsent(nNewDays, new HashSet<>());
                biggestGap.get(nNewDays).add(i + 1); 

                // System.out.println(minDays);
                // System.out.println(biggestGap);
                //this is the portion we should be updating our answer for sure!!
                //the most complicated version of this question should be implemented for sure
                //let's think about it!!!

                //only updating at biggest space for now!!
                //and the very end space for this not including last element transitions!!!
                //alag se handled 
                //no issues!!

                //first if we put it biggest space possible
                int min = biggestGap.lastKey();
                min = min%2 == 0 ? min/2 - 1 : min/2;  //simple no??

                //now update the answer
                // System.out.println(minDays.firstKey());
                minAns = max(minAns, min(minDays.firstKey(), min));  //simple as fuck
                //now what we save it at last, then what??
                //if the current smallest , or the new smallest??
                int lastSpace = d - arr[n-1] - 1;
                if(lastSpace != -1){
                    //then only we should check for sure (think about it for sure bois)
                    minAns = max(minAns, min(minDays.firstKey(), lastSpace));  //simple as fuck
                }

                //now roll back those updates bois!!! please
                minDays.put(nNewDays, minDays.get(nNewDays) - 1);
                //simple updation
                if(minDays.get(nNewDays) == 0){
                    minDays.remove(nNewDays);
                }

                biggestGap.get(nNewDays).remove(i + 1);
                if(biggestGap.get(nNewDays).size() == 0){
                    biggestGap.remove(nNewDays);
                }


                //all these big changes have been rolled back bois
                minDays.putIfAbsent(restDaysToday, 0);
                minDays.put(restDaysToday, minDays.get(restDaysToday) + 1);

                //updating biggest Gap for this question
                biggestGap.putIfAbsent(restDaysToday, new HashSet<>());
                biggestGap.get(restDaysToday).add(i); 

                minDays.putIfAbsent(restDaysTom, 0);
                minDays.put(restDaysTom, minDays.get(restDaysTom) + 1);

                //updating biggest Gap for this question
                biggestGap.putIfAbsent(restDaysTom, new HashSet<>());
                biggestGap.get(restDaysTom).add(i + 1); 

                // System.out.println(minDays);
                // System.out.println(biggestGap);
                
                //simple boss!! all the updates should have been rolled back by now too!!
            }
            ...
            //MAKE YOUR UPDATES IN HERE FOR SURE!!!!
            int min = biggestGap.lastKey();
            min = min%2 == 0 ? min/2 - 1 : min/2;  //simple no??
