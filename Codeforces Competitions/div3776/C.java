...
public class C{
    ...
    public static void main(String[] args) {
        ...
        while(test --> 0){
            ...
            ArrayList<Triplet> val = new ArrayList<>();

            for(int i= 0; i < m; i++){
                data[i][0] = sc.nextInt();
                data[i][1] = sc.nextInt(); 
                val.add(new Triplet(data[i][0], data[i][1], i+1));
            }

            Collections.sort(val, (a,b) -> Integer.compare(a.b, b.b));
            //taking only 2 * n values out of it arranging in something else

            int sum = 0;
            ArrayList<Triplet> points = new ArrayList<>();
            for(int i = 0; i < 2*n; i++){
                points.add(new Triplet(val.get(i).a, val.get(i).b, val.get(i).c));
                sum += val.get(i).b;
        
            }

            Collections.sort(points, (a,b) -> Integer.compare(a.a, b.a));
            //now printing and adding up
            // out.println(val);
            //o
            ArrayList<Integer> pon = new ArrayList<>();
            for(int i = 0; i < 2*n; i++){
                pon.add(points.get(i).c);
            }

            out.println(sum);
            //outputting the segments point number is no
            for(int i= 0; i < n; i++){
                out.println(pon.get(i) + " " + pon.get(2*n - 1 - i));
            }
