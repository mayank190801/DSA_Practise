...
public class three{
    ...
    public static long pos(long[] arr, long val){
    ...
    public static long solve(long[] arr, long index, long total, long reqdef, long reqatt){
        long val = arr[(int)index];
        total -= val;
    ...
    public static void main(String[] args) {
        ...
        while(m --> 0){
            //binary searhc for the index and find both of those vlaues
            //prlong the minimum
            long def = sc.nextLong();
            long att = sc.nextLong();

            long index = pos(arr, def);

            long first = index;
            long second = index + 1;
            
            long valTwo = Long.MAX_VALUE;
            // out.println(valTwo);
            if(second < n){
                valTwo = solve(arr, second, total, def, att);    
            }

            long valOne = solve(arr, first, total, def, att);
            out.println(Math.min(valOne, valTwo));
        }
