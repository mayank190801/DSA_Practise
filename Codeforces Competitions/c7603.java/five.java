...
public class five{
    ...
    public static void main(String[] args) {
        ...
        //similarly try F problem, pretty easy one everybody said 
        while(test -- > 0){
            int n = sc.nextInt();
            long[] arr = new long[n];
            ...
            //check the sum thingy for once
            long num = (n*(n+1))/2;
            long eachSum = sum/num;
            ...
            //now figure out something else
            boolean flag = true;
            LinkedList<Long> ans = new LinkedList<>();
            for(int i = 0; i < n; i++){

                long diff = arr[i] - arr[(i+1)%n];
                if(diff + eachSum <= 0 || (eachSum + diff)%n != 0){
                    flag = false;
                    break;
                }

                ans.add((eachSum+diff)/n);
            }
